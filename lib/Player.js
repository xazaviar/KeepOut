...
function Player(type, opts){
	if(type=="new"){
		...
		this.id 		= opts.id;
		this.authToken	= opts.auth; //Unneed, will remove after players are connecting
		this.name 		= opts.name;
		...
		this.hasMoneyBall = opts.hasMoneyBall;
		...
		this.curTargetBalls = opts.curTargetBalls;
...
Player.prototype.updateOnInput = function(keyboardState, mouseState, gameState){
	...
	else if(this.targetBalls == 0 && this.money >= TARGET_BALL_COST && gameState.sendBall != null){
