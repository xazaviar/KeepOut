...
//Catch up Mechanics
const NEGATIVE_BOOST_SCALING_AMT = parseFloat(process.env.NEGATIVE_BOOST_SCALING_AMT) || 1;
const NEGATIVE_BOOST_SCALING_PER = parseFloat(process.env.NEGATIVE_BOOST_SCALING_PER) || 1000;
...
function Player(type, opts){
	if(type=="new"){
		...
		//Flags
		this.inGame = true; //Indicates if the player is playing the game
		this.lastActive = Date.now(); //Shows when the player was last active
		this.dynamicBackground = true; //Player background color setting
		this.quickStatsEnabled = false; //Player quick stats display setting
		...
		//Flags
		this.inGame = opts.inGame;
		this.lastActive = opts.lastActive;
		this.dynamicBackground = opts.dynamicBackground;
		this.quickStatsEnabled = opts.quickStatsEnabled;
		this.permTarget = null;
		this.change = opts.change;
		this.curTargetBalls = 0;
		this.showLvlUp  = opts.showLvlUp;
		this.showRoundEnd = opts.showRoundEnd;
...
Player.prototype.updateOnInput = function(keyboardState, mouseState, gameState){
	...
	//Purchase a target ball
	if(gameState.storePurchase == "TARGET_BALL" && this.money >= parseInt(process.env.TARGET_BALL_COST)){
		this.money -= parseInt(process.env.TARGET_BALL_COST);
		this.change = "self";
		this.targetBalls++;
	}
	...
	//Change Name
	if(gameState.nameChange!=null){
		var name = gameState.nameChange;
		name = name.trim(); //Remove surrounding whitespace
        name = name.replace(/</g, "").replace(/>/g, "").replace(/'/g, "").replace(/"/g, ""); //Remove illegal characters
	...
	this.dynamicBackground = gameState.dynamicBackground;
	this.quickStatsEnabled = gameState.quickStatsEnabled;
	this.lastActive = Date.now();
	// if(this.showRoundEnd) console.log(this.showRoundEnd);
	if(gameState.gotLvlup == true) this.showLvlUp = false;
	if(gameState.gotRoundEnd == true) this.showRoundEnd = false;
}

Player.prototype.update = function(){
	var scoreAutoModifier = Math.floor(this.score/NEGATIVE_BOOST_SCALING_PER)*-1*NEGATIVE_BOOST_SCALING_AMT+NEGATIVE_BOOST_SCALING_AMT;

	//Check items
	if(this.activeBoost.dur > 0) this.activeBoost.dur--;
	else if(this.score < 0) this.activeBoost = {amt:scoreAutoModifier,dur:-1}; 
	...
	if(this.lifetime%MONEY_BALL_CHECK==0 && !this.hasMoneyBall){
		...
		if(r <= this.moneyBallChance){
			...
			this.moneyBallCount++;
			this.newBall(Ball.create("MONEY-MONEY-MONEY",{name:process.env.GAME_SENDER,id:"-1"},"MONEY"));
			this.change = "self";
		}
...
Player.prototype.levelUp = function(){
	...
	if(this.level == 1){//Give Settings with quickStats
		...
		this.menu[4].extras.nameChange = true; 
		this.money += 100;
		this.moneyTotal += 100;
	} 
	...
	if(this.level == 10){
		this.money += 2500;
		this.moneyTotal += 2500;
		this.targetBalls += 10;
	} 
	if(this.level == 11){
		this.money += 3000;
		this.moneyTotal += 3000;
		this.targetBalls += 15;
	} 
	if(this.level == 12){
		this.money += 4000;
		this.moneyTotal += 4000;
		this.targetBalls += 20;
	}
	if(this.level > 10){
	...
	for(var b in this.balls){
		if(this.balls[b].auth == this.ballReturn.ball.auth){
			...
			if(this.permTarget!=null)
				...
				this.balls[b].changeSender({name:process.env.GAME_SENDER, id:-1});
...
Player.prototype.removeNormalBall = function(){
	//Call this only when removing a ball due to there being too many balls
	for(var b in this.balls){
		if(this.balls[b].type=="NORMAL"){
			this.swatCount++;
			...
			for(var h = this.ballHistory.length-1; h > -1; h--){
				if(this.balls[b].auth == this.ballHistory[h].auth){
					this.ballHistory[h].removed = Date.now();
					this.ballHistory[h].autoSwat = false;
					break;
				}
			}
...
Player.prototype.verifyInGame = function(inactive_time){
	// console.log("VERIFYING: "+this.name+" | "+new Date(Date.now()).toLocaleString()+" - "+new Date(this.lastActive).toLocaleString()+" > "+inactive_time);
	if(Date.now() - this.lastActive > inactive_time){
...
Player.prototype.resetRound = function(ranking){
	this.cleanHistory();
    this.balls = [];
    this.score = 0;

    if(ranking){
    	this.showRoundEnd = ranking;
    	if(ranking.rank==1) 	this.showRoundEnd["message"] = "You won the game!";
    	else if(ranking.rank<4)	this.showRoundEnd["message"] = "Congratulations!";
    	else if(ranking.rank<6)	this.showRoundEnd["message"] = "You made the top 5!";
    	else if(ranking.rank<11)this.showRoundEnd["message"] = "You made the top 10!";
    	else					this.showRoundEnd["message"] = "Better luck next round.";
    }
}
